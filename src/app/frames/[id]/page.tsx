...
export function generateStaticParams() {
  return productsByCategory.frames.map((product) => ({ id: product.id }));
}

export default function FrameDetailPage({ params }: { params: { id: string } }) {
  const product = getProduct("frames", params.id);
    ...
    <div className="page-shell flex flex-col gap-16 py-32 md:py-36">
      <section className="grid gap-12 lg:grid-cols-[1fr_0.95fr] lg:items-start py-5">
        <div className="card-surface overflow-hidden rounded-[var(--radius-lg)] bg-white">
          <div className="relative h-[460px] bg-slate-900/5">
        ...
        <div className="space-y-10">
          ...
          {product.highlights?.length ? (
            <div>
              <h2 className="text-2xl font-semibold text-slate-900">Kenapa rider memilih produk ini?</h2>
          ...
          <div className="rounded-[var(--radius-lg)] bg-white/90 p-6 text-sm leading-relaxed text-slate-600 shadow-sm backdrop-blur">
            <p>
              Ingin paket lengkap dengan wheelset dan komponen?{" "}
              <Link href="/catalog" className="font-semibold text-slate-900 transition hover:text-[var(--accent-foreground)]">
                Lihat katalog
              </Link>{" "}
              atau konsultasi langsung dengan tim kami melalui WhatsApp.
            </p>
          </div>
        </div>
      </section>
