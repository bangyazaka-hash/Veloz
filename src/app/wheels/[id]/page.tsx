...
export function generateStaticParams() {
  return productsByCategory.wheels.map((product) => ({ id: product.id }));
}

export default function WheelDetailPage({ params }: { params: { id: string } }) {
  const product = getProduct("wheels", params.id);
      ...
      <section className="grid gap-12 lg:grid-cols-[1fr_0.95fr] lg:items-start py-5">
        ...
        <div className="space-y-10">
          ...
          {product.highlights?.length ? (
            <div>
              <h2 className="text-2xl font-semibold text-slate-900">Keunggulan wheelset</h2>
